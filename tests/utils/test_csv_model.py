...
def test_csv_model_write_obj2():
    ...
    data = [
        People(name="Alice", info=TestModel(age=20, address="Shanghai", data={"a": 123})),
        People(name="Bob", info=TestModel(age=30, address=None)),
    ]

    header = TestModel.flatten_header(data[0])
    assert header == ['name', 'info.age', 'info.address', 'info.data']

    row = TestModel.flatten_data(data[0])
    assert row == {'name': 'Alice', 'info.age': 20, 'info.address': 'Shanghai', 'info.data': '{"a":123}'}
    row = TestModel.flatten_data(data[1])
    assert row == {'name': 'Bob', 'info.age': 30, 'info.address': None, 'info.data': None}
